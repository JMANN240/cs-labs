...
using std::string; using std::map; using std::transform;

// factory
class LogoMaker{
public:
    static string* makeLogo(string a){
        if(logos_.find(a) == logos_.end()) {
            string upper = a;
            transform(upper.begin(), upper.end(), upper.begin(), ::toupper);
            logos_[a] = new string(upper);
        }
        return logos_[a];
    }
private:
    static std::map<string, string*> logos_;
}; 

std::map<string, string*> LogoMaker::logos_;
...
public:
   ...
   void receive(const string &msg){
      if (msg.find(id_) != string::npos || msg.find("all") != string::npos){
	 if(msg.find("clear") != string::npos){
	    ...
	    cout << "Tower, this is " << id_ << " please repeat." << endl;
      }
   }
   ...
   void proceed(){
      ...
      cout << id_ << 
       ...
       cout << "..." << *(LogoMaker::makeLogo(company_)) << flightNo_ << (task_ == Task::taxiing ? " takes off" : " lands") << "..." << endl;
