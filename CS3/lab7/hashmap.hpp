...
hashmap<Key, Value, Compare, Hash>::insert(const Element& x) {
   ...
   if (it == elems_[bucket].end()) {
      ...
      return make_pair(&(*(--elems_[bucket].end())), true);
   } else {
...
hashmap<Key, Value, Compare, Hash>::erase(const Key& x) {
   ...
   if (it != elems_[bucket].end()) {    // the element exists, erase it
      ...
      if (size_ == 0) {
         ...
         for (size_t i = bucket; i < hash_.numBuckets(); ++i) {
            if (!elems_[i].empty()) {
...
Value& hashmap<Key, Value, Compare, Hash>::operator[] (const Key& x) {
   ...
   if (found == nullptr) { // if key not found, create new element with empty value
      auto insertResult = insert(make_pair(x, Value())); // calling default constructor on Value
...
void hashmap<Key, Value, Compare, Hash>::rehash(size_t n) {
    if (n > hash_.numBuckets()) {
        hashmap<Key, Value> newHashmap(Compare(), n);
        cout << newHashmap.hash_.numBuckets() << endl;
        for (auto l: elems_) {
            for (auto p: l) {
                newHashmap.insert(p);
                cout << "inserting <" << p.first << ", " << p.second << "> into newHashmap" << endl;
            }
        }
        hash_ = newHashmap.hash_;
        size_ = newHashmap.size_;
        comp_ = newHashmap.comp_;
        elems_.swap(newHashmap.elems_);
    }
}