...
public:
    ...
    void show() override { cout << "erase line " << line_; }
private:
    int line_;
    string str_;
};
...
public:
    ...
    void history() {
        std::stack<Command*> tempStack;
        Command* tempCommand;
        int i = 1;
        while (!doneCommands_.empty()) {
            tempCommand = doneCommands_.top();
            doneCommands_.pop();
            cout << i << ". ";
            tempCommand->show();
            cout << endl;
        ...
        while (!tempStack.empty()) {
            doneCommands_.push(tempStack.top());
            tempStack.pop();
        }
    }

    void redo(int index) {
        if (index > doneCommands_.size()) {
            cout << "Cannot redo command that does not exist" << endl;
            return;
        }
        std::stack<Command*> tempStack;
        Command* tempCommand;
        ...
        tempCommand->execute();
        doneCommands_.push(tempCommand);
    }
