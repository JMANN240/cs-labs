...
AST::~AST() {
  ...
  for(AST* node : child) delete node;
}
...
AST::AST(const AST& actual) {
    ...
    //into this.
    for(AST* node : actual.child) {
        this->child.push_back(new AST(*node));
    }
...
void AST::swap(AST& b) {
    ...
    nodes tmpNodeType;
    std::string tmpTag, tmpCloseTag;
    std::list<AST*> tmpChild;
    std::string tmpText;
    tmpNodeType = b.nodeType;
    tmpTag = b.tag;
    tmpCloseTag = b.closeTag;
    tmpChild = b.child;
    tmpText = b.text;
    b.nodeType = nodeType;
    b.tag = tag;
...
void AST::mainHeader(const std::vector<std::string>& profileName) {
    ...
    AST* includeTree = new AST(token, "#include \"profile.hpp\"\n");
    ...
    for (std::list<AST*>::iterator iter = child.begin(); iter != child.end(); iter++) {
        ...
        if ((*iter)->tag == "function") {
            ...
            child.insert(iter,includeTree);
            for (std::string name : profileName) {
                std::string pfName = name;
                std::replace(name.begin(), name.end(), '_', '.');
                profileTree = new AST(token, "profile " + pfName + "(\"" + name + "\");\n");
                child.insert(iter, profileTree);
            }
            std::cout << "HELP40" << std::endl;
            
            std::cout << "HELP44" << std::endl;
            break;
        }
    }
}
...
void AST::fileHeader(const std::string& profileName) {
    ...
    std::cout << "HELP10" << std::endl;
    AST* includeTree = new AST(token, "#include \"profile.hpp\"\n");
    std::cout << "HELP11" << std::endl;
    AST* profileTree = new AST(token, "extern profile " + profileName + ";\n");
    std::cout << "HELP12" << std::endl;
    for (std::list<AST*>::iterator iter = child.begin(); iter != child.end(); iter++) {
        std::cout << "HELP13" << std::endl;
        if ((*iter)->tag == "function") {
            child.insert(iter,includeTree);
            std::cout << "HELP14" << std::endl;
            child.insert(iter,profileTree);
            std::cout << "HELP18" << std::endl;
            
            std::cout << "HELP19" << std::endl;
            break;
        }
    }
}
